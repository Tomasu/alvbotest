...
VBO::VBO(CUSTOM_VERTEX *data, uint32_t size) : vbo_(0), size_(size)
{
	...
	Debug("new VBO: size:%.02fMB", ((double)size_*sizeof(*data))/1024.0/1024.0);
}
...
{
	al_draw_vertex_buffer(vbo_, 0, 0, size_, ALLEGRO_PRIM_TRIANGLE_LIST);
}

VBO *VBO::Create()
{
	uint32_t DATA_SIZE = 16 * 16 * 16 * 36;
	
	CUSTOM_VERTEX *data = new CUSTOM_VERTEX[DATA_SIZE];
	if(!data)
		return nullptr;
	
	memset(data, 0, sizeof(CUSTOM_VERTEX) * DATA_SIZE);
	...
	for(int dy = 0; dy < 16; dy++)
	{
		for(int dz = 0; dz < 16; dz++)
		{
			for(int dx = 0; dx < 16; dx++)
			{
				...
				dptr += num_idx;
				total_size += num_idx;
				if(total_size >= DATA_SIZE*sizeof(CUSTOM_VERTEX))
					Debug("total_size(%i) > DATA_SIZE(%i)", total_size, DATA_SIZE*sizeof(CUSTOM_VERTEX));
